...
const CableChart: React.FC<CableChartProps> = ({
  ...
  const fetchCablePCPData = async () => {
    ...
    try {
      ...
      const {
        data: tarefas,
        error: tarefasError
      } = await supabase.from('tarefas').select('cabo, percentual_executado').eq('obra_id', obraId).eq('semana', formattedDate) // Filter by selected week
  ...
  const COLORS = ['#10b981', '#27c08e', '#3ec69c', '#55cda9', '#6cd3b6', '#83d9c4', '#9adfd1', '#b1e5df', '#c8ebec', '#dff1f9'];
  return <Card className="w-full">
      <CardHeader className="pb-2">
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={300}>
          ...
          bottom: 5
            ...
            <XAxis type="number" domain={[0, 100]} tickFormatter={value => `${value}%`} tick={{
            fontSize: 12
          }} />
            <YAxis type="category" dataKey="name" width={70} tick={{
            fontSize: 12
          }} />
            <Tooltip formatter={(value: number) => [`${value.toFixed(1)}%`, 'PCP']} labelFormatter={name => `Cabo: ${name}`} />
            <Bar dataKey="value" name="PCP" radius={[0, 4, 4, 0]}>
