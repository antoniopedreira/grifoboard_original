...
const WeeklyCausesChart: React.FC<WeeklyCausesChartProps> = ({
  weekStartDate,
  tasks
}) => {
  ...
  useEffect(() => {
    // Current week causes
    const currentWeekCauses = tasks.filter(task => task.causeIfNotDone).reduce((acc, task) => {
  ...
  return <div className="h-full">
      ...
      <div className="space-y-2 max-h-64 overflow-y-auto">
        {causesData.length === 0 ? <div className="text-center py-8 text-muted-foreground">
          ...
          </div> : causesData.map((item, index) => <div key={index} onClick={() => handleCauseClick(item)} className={`p-3 rounded-lg border transition-colors hover:bg-muted/30 cursor-pointer ${item.isCritica ? 'border-destructive/30 bg-destructive/5' : 'border-border'}`}>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-medium text-sm">{item.causa}</span>
                    {item.isCritica && <Badge variant="destructive" className="text-xs">Crítica</Badge>}
                  </div>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {selectedCause?.isCritica && <Badge variant="destructive" className="text-xs">Crítica</Badge>}
            </DialogTitle>
          </DialogHeader>
          
          {selectedCause && <div className="space-y-3 mt-4">
              <div className="text-sm text-muted-foreground mb-4">
                Total de atividades: {selectedCause.quantidade}
              </div>
              
              {selectedCause.tasks.map((task, index) => <div key={index} className="border rounded-lg p-4 space-y-2">
