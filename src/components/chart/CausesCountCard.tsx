...
const CausesCountCard: React.FC<CausesCountCardProps> = ({ tasks, onCauseSelect }) => {
  ...
  if (causesArray.length === 0) {
    return (
      <div className="bg-white rounded-2xl shadow-[0_2px_6px_rgba(0,0,0,0.08)] hover:shadow-[0_4px_12px_rgba(0,0,0,0.12)] transition-all duration-300 animate-fade-in p-4">
    ...
    <div className="bg-white rounded-2xl shadow-[0_2px_6px_rgba(0,0,0,0.08)] hover:shadow-[0_4px_12px_rgba(0,0,0,0.12)] transition-all duration-300 animate-fade-in p-4">
      ...
      <div className="space-y-2">
        {causesArray.map(({ cause, count }, index) => (
          <div 
            key={cause}
            className="flex items-center justify-between py-2 cursor-pointer hover:bg-slate-50 rounded-lg px-2 transition-all duration-200 animate-fade-in"
            style={{ 
          ...
          >
            <span 
              className="text-slate-700 font-medium text-sm flex-1 mr-2"
              style={{ 
                whiteSpace: 'normal',
                wordWrap: 'break-word'
              }}
