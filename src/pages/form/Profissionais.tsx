...
const DISPONIBILIDADE_ATUAL = [
  "Imediata",
  "Em 15 dias",
  "Em 30 dias",
  "Apenas por contrato pontual"
];

const MODALIDADE_TRABALHO = [
  "CLT",
  "PJ",
  "Pessoa física / diária",
  "Freelancer por obra"
];
...
const FormProfissionais = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("formulario_profissionais").insert({
  ...
  return (
    <FormTemplate 
      ...
      subtitle="Formulário de cadastro para profissionais terceirizados"
    >
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <p className="text-sm text-muted-foreground">
          Engenheiros, Mestres, Pedreiros, Técnicos, etc.
        </p>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            >
              <option value="">Selecione...</option>
              {FUNCOES_PRINCIPAIS.map(funcao => (
                <option key={funcao} value={funcao}>{funcao}</option>
              ))}
            </select>
          </div>

          {formData.funcaoPrincipal === "Outro" && (
            <div>
              <Label htmlFor="funcaoPrincipalOutro">Especifique a função</Label>
              <Input
                id="funcaoPrincipalOutro"
                value={formData.funcaoPrincipalOutro}
                onChange={(e) => setFormData({ ...formData, funcaoPrincipalOutro: e.target.value })}
              />
            </div>
          )}
              ...
              {ESPECIALIDADES.map(especialidade => (
                <div key={especialidade} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={() => handleCheckboxChange("especialidades", especialidade)}
                  />
          ...
          {formData.especialidades.includes("Outra") && (
            <div>
              <Label htmlFor="especialidadesOutro">Especifique a especialidade</Label>
              <Input
                id="especialidadesOutro"
                value={formData.especialidadesOutro}
                onChange={(e) => setFormData({ ...formData, especialidadesOutro: e.target.value })}
              />
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Textarea
              id="obrasRelevantes"
              placeholder="Descreva brevemente as obras mais relevantes..."
              value={formData.obrasRelevantes}
              onChange={(e) => setFormData({ ...formData, obrasRelevantes: e.target.value })}
              rows={4}
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              id="disponibilidadeAtual"
              required
              value={formData.disponibilidadeAtual}
              onChange={(e) => setFormData({ ...formData, disponibilidadeAtual: e.target.value })}
              className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
              <option value="">Selecione...</option>
              {DISPONIBILIDADE_ATUAL.map(disp => (
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, modalidadeTrabalho: e.target.value })}
              className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
              <option value="">Selecione...</option>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
              {REGIOES_BRASIL.map(regiao => (
                <div key={regiao} className="flex items-center space-x-2">
                  <Checkbox
                    id={`reg-${regiao}`}
                    checked={formData.regioesAtendidas.includes(regiao)}
                    onCheckedChange={() => handleCheckboxChange("regioesAtendidas", regiao)}
                  />
          ...
          <div>
            ...
            <Input
              ...
              value={formData.cidadesFrequentes}
              onChange={(e) => setFormData({ ...formData, cidadesFrequentes: e.target.value })}
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              id="pretensaoValor"
              required
              placeholder="Ex: R$ 5.000/mês ou R$ 200/dia"
              value={formData.pretensaoValor}
              onChange={(e) => setFormData({ ...formData, pretensaoValor: e.target.value })}
            />
          </div>
          ...
          <div>
            ...
            <select
              id="equipamentosProprios"
              required
              value={formData.equipamentosProprios}
              onChange={(e) => setFormData({ ...formData, equipamentosProprios: e.target.value })}
              className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
              <option value="">Selecione...</option>
              {EQUIPAMENTOS_PROPRIOS.map(equip => (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
              {DIFERENCIAIS.map(diferencial => (
                <div key={diferencial} className="flex items-center space-x-2">
                  <Checkbox
                    id={`dif-${diferencial}`}
                    checked={formData.diferenciais.includes(diferencial)}
                    onCheckedChange={() => handleCheckboxChange("diferenciais", diferencial)}
                  />
          ...
          {formData.diferenciais.includes("Outro") && (
            <div>
              ...
              <Input
                id="diferenciaisOutro"
                value={formData.diferenciaisOutro}
                onChange={(e) => setFormData({ ...formData, diferenciaisOutro: e.target.value })}
              />
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          <FormSectionHeader 
            label="7. Documentos e Portfólio" 
            description="(Uploads de arquivos serão habilitados em breve)"
          />
