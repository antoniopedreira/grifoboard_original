...
import { Trophy, Zap, Target, Shield, BookOpen, Crown, Flame, CheckCircle2, Medal, TrendingUp, Users, Loader2 } from "lucide-react";
...
const LevelBadge = ({
  ...
  return <Badge className={cn("px-3 py-1 text-xs font-bold uppercase tracking-wider", color)}>
      Nível {level} - {label}
    </Badge>;
};
const XPProgressBar = ({
  ...
  return <div className="space-y-2">
      ...
      <div className="relative h-4 w-full bg-slate-100 rounded-full overflow-hidden border border-slate-200">
        <div className="h-full bg-gradient-to-r from-yellow-500 to-[#C7A347] transition-all duration-1000 ease-out" style={{
        width: `${percentage}%`
      }} />
      </div>
...
const GrifoWay = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const [profData, rankData] = await Promise.all([gamificationService.getProfile(userSession.user.id), gamificationService.getRanking()]);
  ...
  return <div className="min-h-screen bg-slate-50/50 pb-20">
        ...
        <Tabs defaultValue="dashboard" value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <div className="flex justify-center md:justify-start">
            <TabsList className="bg-white shadow-lg border border-slate-100 p-1 rounded-xl h-auto">
              <TabsTrigger value="dashboard" className="px-6 py-3 rounded-lg data-[state=active]:bg-[#112131] data-[state=active]:text-white transition-all gap-2 flex items-center">
                <Trophy className="h-4 w-4" />
                Meu Desempenho
              </TabsTrigger>
              <TabsTrigger value="manual" className="px-6 py-3 rounded-lg data-[state=active]:bg-[#112131] data-[state=active]:text-white transition-all gap-2 flex items-center">
              ...
              <div className="lg:col-span-2">
                <Card className="border-none shadow-xl h-full flex flex-col">
                  ...
                  <CardContent className="flex-1 p-0">
                    {ranking.length === 0 ? <div className="p-8 text-center text-slate-500">
                        Nenhum colaborador pontuou ainda. Seja o primeiro!
                      </div> : <div className="divide-y divide-slate-100">
                        {ranking.map((user, index) => <div key={user.id} className={cn("flex items-center gap-4 p-4 transition-colors hover:bg-slate-50", user.id === userSession?.user?.id && "bg-blue-50/50")}>
                            <div className="flex-shrink-0 w-8 text-center font-bold text-slate-400">
                              {index === 0 ? <Medal className="h-6 w-6 text-yellow-500 mx-auto" /> : index === 1 ? <Medal className="h-6 w-6 text-slate-400 mx-auto" /> : index === 2 ? <Medal className="h-6 w-6 text-amber-700 mx-auto" /> : `#${index + 1}`}
                            </div>
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {user.id === userSession?.user?.id && <Badge variant="outline" className="text-[10px] h-4 px-1">
                                    Você
                                  </Badge>}
                              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              <div className="md:col-span-1 space-y-4">
                <Card className="sticky top-24 border-none shadow-md">
                  ...
                  <CardContent className="space-y-1 p-2">
                    {["Introdução", "Pilares FAST 2.0", "Valores Culturais", "Comportamentos"].map((item, i) => <Button key={item} variant="ghost" className="w-full justify-start text-slate-600 hover:text-[#112131] hover:bg-slate-50" onClick={() => document.getElementById(`section-${i}`)?.scrollIntoView({
                    behavior: "smooth"
                  })}>
                        {i + 1}. {item}
                      </Button>)}
              ...
              <div className="md:col-span-3 space-y-8 pb-20">
                ...
                <div id="section-2">
                  ...
                  }].map((val, i) => <Card key={i} className="hover:shadow-md transition-shadow border-l-2 border-l-slate-200 hover:border-l-[#C7A347]">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      ...
                      <ul className="space-y-2">
                        {["Atualiza o GrifoBoard todos os dias", "Faz checklist FAST diário", "Mantém comunicação clara"].map((item, i) => <li key={i} className="flex items-center gap-2 text-sm text-slate-700 bg-green-50 p-2 rounded-md border border-green-100">
